            ...
            <form onSubmit={submit} className="space-y-10">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-8 p-6 rounded-[2rem] bg-slate-50/50 border border-slate-100">
                    <div className="relative group">
                        <div className="h-28 w-28 rounded-full overflow-hidden border-4 border-white shadow-xl shadow-indigo-100 bg-white flex items-center justify-center">
                            {/* Logika Tampilan: Jika flag hapus TRUE, tampilkan icon User default */}
                            {data.delete_photo ? (
                                ...
                                <img src={`/storage/${user.profile_photo_path}`} className="h-full w-full object-cover" />
                            ) : (
                                <div className="h-full w-full bg-indigo-50 flex items-center justify-center text-indigo-300">
                                    <UserIcon size={48} />
                                </div>
                    ...
                    <div className="text-center sm:text-left space-y-2">
                        <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Foto Profil</h4>
                        <div className="flex items-center justify-center sm:justify-start gap-4 pt-2">
                            <button type="button" onClick={() => photoInput.current.click()} className="text-[11px] font-black text-indigo-600 uppercase tracking-widest">Pilih Foto</button>
                            
                            {/* Tombol Hapus: Hanya muncul jika ada foto lama DAN belum ditandai hapus */}
                            {!data.delete_photo && (user.profile_photo_path || photoPreview) && (
                                <button type="button" onClick={markPhotoForDeletion} className="text-[11px] font-black text-rose-500 uppercase tracking-widest">Hapus</button>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4 pt-4">
                    <PrimaryButton disabled={processing} className="bg-indigo-600 px-8 py-4 rounded-2xl shadow-lg shadow-indigo-100">
                    ...
                    <Transition
                        ...
                        leaveTo="opacity-0"
                    >
                        <p className="text-sm font-bold text-emerald-600 italic">Tersimpan.</p>
                    </Transition>
                </div>
            </form>
        </section>
